...
Monolith.uplugin
  ...
  MonolithReflectionIntel — Deterministic reflection intelligence layer. v0.17.0 Phases 1 + 2 + 3a ship 16 actions total: `decision_query` (5 — markdown decision-record mining), `risk_query` (5 — git churn / co-change / hotspot scoring / conditional-gate inventory), `cppreflect_query` (5 — UE 5.7 reflection-edge queries from UHT artefacts cross-joined with `IAssetRegistry`), plus `source_query("audit_module_dep_reality")` (1 — UPROPERTY/Build.cs missing-dep audit). All five indexers write into `EngineSource.db` with lazy bootstrap + hot-reload refresh. Phase 3b (tree-sitter native-tag tracking) and Phase 4 (network) are WISHLIST.
```
...
| 3.18 | MonolithReflectionIntel | [specs/SPEC_MonolithReflectionIntel.md](specs/SPEC_MonolithReflectionIntel.md) | Deterministic, $0-LLM reflection intelligence — Decision + Risk + CppReflect + Module-Dep Reality Audit. v0.17.0 ships 16 actions across `decision_query` (5), `risk_query` (5), `cppreflect_query` (5), and `source_query("audit_module_dep_reality")` (1). Phase 3b native-tag tracking (tree-sitter) and Phase 4 network are WISHLIST. |
...
| MonolithReflectionIntel | cppreflect | 5 | `get_uclass`, `list_uproperties`, `list_ufunctions`, `find_interface_impls`, `find_class_specifier`. UE 5.7 reflection-edge queries driven by direct reads of UHT artefacts (`Intermediate/Build/Win64/.../Inc/<Module>/UHT/*.gen.cpp`) cross-joined with `IAssetRegistry::GetDependencies` — no tree-sitter dependency, no ThirdParty vendoring. Writes into 6 new SQLite tables on `EngineSource.db`: `reflect_uclasses`, `reflect_uproperties`, `reflect_ufunctions`, `reflect_uinterfaces`, `reflect_uinterface_impls`, `cpp_asset_edges`. All five carry `readOnlyHint + idempotentHint`. `list_uproperties`, `list_ufunctions`, `find_interface_impls`, `find_class_specifier` cursor-paginated. Phase 3a of Reflection Intelligence — v0.17.0 (`Plugins/Monolith/Docs/specs/SPEC_MonolithReflectionIntel.md` §5). Phase 3b native-tag tracking (`reflect_native_tag_decls` / `reflect_native_tag_externs` + `list_native_tags`) deferred — see §5b. |
| **Total** | | **1374** in-tree active actions across 22 in-tree namespaces (17 module namespaces + 5 framework: `monolith` meta, `level_sequence`, `bulk_fill`, `describe`, plus the three Reflection Intel namespaces). v0.17.0 adds `decision` (5 actions, Phase 1) + `risk` (5 actions, Phase 2) + `cppreflect` (5 actions, Phase 3a) + 1 `source` audit action (Phase 2 cross-namespace registration `audit_module_dep_reality`) from the new `MonolithReflectionIntel` module — namespace count 19 → 22, in-tree total 1358 → 1374. Reconciled 2026-05-28: the per-namespace rows above are authoritative; the prior deferred-batch list (+1/+3/+4/+1/+17/+4/+9/+1 = +40 over the stale 1318 baseline) is now folded in. 45 town-gen experimental actions disabled by default → 1419 with `bEnableProceduralTownGen=true`. Without `WITH_COMMONUI`: 1319 (1364 w/ town gen — strips the 51 CommonUI actions then adds back the unconditional 11); without `WITH_GBA`: 1239 (1284 w/ town gen — strips the 135 GAS actions); without both: 1184 (1229 w/ town gen). Without `WITH_METASOUND`: 1362 (the 12 audio MetaSound introspection actions become inert; the `audio` bulk_fill adapter's MetaSound paths gate the same). The `ui` namespace double-counts 4 aliased GAS actions; **distinct** action handlers = **1370** in default-active configuration, **1415** with town gen registered. **Sibling plugins are NOT counted in the totals above** (per `monolith-release.md` Action Count Discipline) — the public Monolith release zip does not include them. For transparency, live `monolith_discover()` (which includes siblings if installed) returns **1597** = 1374 in-tree + 11 `claudedesign` (`MonolithClaudeDesignBridge`) + 158 `inventory` (`MonolithISX`) + 28 `steam` (`MonolithSteamBridge`) + 26 `substance` (`MonolithSubstance`); the 223 sibling actions are advisory only and never affect the public-release headline. | Namespace count rose 18 → 19 with `level_sequence` (v0.14.8); v0.14.11 [Unreleased] lit up the existing `bulk_fill` + `describe` dispatchers' adapters without adding new namespaces; v0.17.0 brings three new namespaces from MonolithReflectionIntel — `decision` (Phase 1), `risk` (Phase 2), `cppreflect` (Phase 3a) — for 19 → 22, plus a +1 audit on the existing `source` namespace. The 2026-05-22 `monolith_guide` addition stayed inside the existing `monolith` meta-namespace. Sibling-plugin namespaces are documented in their own repos and are NOT in the public Monolith release zip — sibling totals are advisory only |
