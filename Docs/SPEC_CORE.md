...
Monolith.uplugin
  ...
  MonolithReflectionIntel — Deterministic reflection intelligence layer. v0.17.0 Phases 1 + 2 + 3a + 4a shipped 26 actions; the [Unreleased] `cppreflect_query("list_class_specifiers")` follow-up brings it to 27: `decision_query` (5 — markdown decision-record mining), `risk_query` (5 — git churn / co-change / hotspot scoring / conditional-gate inventory), `cppreflect_query` (6 — UE 5.7 reflection-edge queries from UHT artefacts cross-joined with `IAssetRegistry`, incl. the [Unreleased] `list_class_specifiers` token-discovery action), `network_query` (4 — replicated-class / RPC / OnRep enumeration + unbalanced-OnRep audit), `pipeline_query` (2 — `pr_review` + `release_readiness` composers), plus `source_query("audit_module_dep_reality")` (1 — UPROPERTY/Build.cs missing-dep audit) and 4 audit actions on existing namespaces (`material_query("audit_orphan_materials")`, `niagara_query("audit_cross_asset_refs")`, `blueprint_query("audit_cdo_drift")`, `project_query("audit_orphan_assets")`). All six indexers write into `EngineSource.db` with lazy bootstrap + hot-reload refresh. Phase 3b (tree-sitter native-tag tracking) and Phase 4b (tag-graph + thread-safety audits + bare `UPROPERTY(Replicated)` detection) are WISHLIST — both blocked on tree-sitter substrate.
```
...
| **Total** | | **1385** in-tree active actions across 24 in-tree namespaces (17 module namespaces + 7 framework: `monolith` meta, `level_sequence`, `bulk_fill`, `describe`, plus the five Reflection Intel namespaces — `decision`, `risk`, `cppreflect`, `network`, `pipeline`). v0.17.0 adds `decision` (5 actions, Phase 1) + `risk` (5 actions, Phase 2) + `cppreflect` (6 actions, Phase 3a + an [Unreleased] `list_class_specifiers` follow-up) + `network` (4 actions, Phase 4a) + `pipeline` (2 actions, Phase 4a) + 1 `source` audit action (Phase 2 cross-namespace registration `audit_module_dep_reality`) + 4 cross-namespace audit actions on `material` / `niagara` / `blueprint` / `project` (Phase 4a) — namespace count 19 → 24, in-tree total 1358 → 1384 (Phases 1+2+3a = +16; Phase 4a = +10 actions: 4 network + 2 pipeline + 4 cross-namespace audits), then [Unreleased] 1384 → 1385 (+1 cppreflect `list_class_specifiers`). Reconciled 2026-05-28: the per-namespace rows above are authoritative; the prior deferred-batch list (+1/+3/+4/+1/+17/+4/+9/+1 = +40 over the stale 1318 baseline) is now folded in. 45 town-gen experimental actions disabled by default → 1430 with `bEnableProceduralTownGen=true`. Without `WITH_COMMONUI`: 1330 (1375 w/ town gen — strips the 51 CommonUI actions then adds back the unconditional 11); without `WITH_GBA`: 1250 (1295 w/ town gen — strips the 135 GAS actions); without both: 1195 (1240 w/ town gen). Without `WITH_METASOUND`: 1373 (the 12 audio MetaSound introspection actions become inert; the `audio` bulk_fill adapter's MetaSound paths gate the same). The `ui` namespace double-counts 4 aliased GAS actions; **distinct** action handlers = **1381** in default-active configuration, **1426** with town gen registered. **Sibling plugins are NOT counted in the totals above** (per `monolith-release.md` Action Count Discipline) — the public Monolith release zip does not include them. For transparency, live `monolith_discover()` (which includes siblings if installed) returns **1608** = 1385 in-tree + 11 `claudedesign` (`MonolithClaudeDesignBridge`) + 158 `inventory` (`MonolithISX`) + 28 `steam` (`MonolithSteamBridge`) + 26 `substance` (`MonolithSubstance`); the 223 sibling actions are advisory only and never affect the public-release headline. | Namespace count rose 18 → 19 with `level_sequence` (v0.14.8); v0.14.11 [Unreleased] lit up the existing `bulk_fill` + `describe` dispatchers' adapters without adding new namespaces; v0.17.0 Phases 1+2+3a brought three new namespaces from MonolithReflectionIntel — `decision`, `risk`, `cppreflect` — for 19 → 22, plus a +1 audit on the existing `source` namespace. v0.17.0 Phase 4a brings two more — `network`, `pipeline` — for 22 → 24, plus 4 cross-namespace audit actions on existing `material` / `niagara` / `blueprint` / `project` namespaces. The 2026-05-22 `monolith_guide` addition stayed inside the existing `monolith` meta-namespace. Sibling-plugin namespaces are documented in their own repos and are NOT in the public Monolith release zip — sibling totals are advisory only |
