...
Monolith.uplugin
  ...
  MonolithReflectionIntel — Deterministic reflection intelligence layer. v0.17.0 Phases 1 + 2 ship 11 actions total: `decision_query` (5 — markdown decision-record mining), `risk_query` (5 — git churn / co-change / hotspot scoring / conditional-gate inventory), plus `source_query("audit_module_dep_reality")` (1 — UPROPERTY/Build.cs missing-dep audit). All four indexers write into `EngineSource.db` with lazy bootstrap + hot-reload refresh. Phase 3 cppreflect / Phase 4 network are planned WISHLIST.
```
...
| All others (17 Editor + 1 Runtime) | Default | Editor (MonolithBABridge is optional — empty shell when Blueprint Assist absent). MonolithReflectionIntel ships v0.17.0 Phases 1 + 2 of Reflection Intelligence — `decision_query` + `risk_query` namespaces plus a `source_query("audit_module_dep_reality")` audit action. MonolithAudioRuntime is Type: Runtime — provides `UMonolithSoundPerceptionUserData` + `UMonolithAudioPerceptionSubsystem` for `audio::bind_sound_to_perception` |
...
| 3.18 | MonolithReflectionIntel | [specs/SPEC_MonolithReflectionIntel.md](specs/SPEC_MonolithReflectionIntel.md) | Deterministic, $0-LLM reflection intelligence — Decision + Risk + Module-Dep Reality Audit. v0.17.0 ships 11 actions across `decision_query` (5), `risk_query` (5), and `source_query("audit_module_dep_reality")` (1). Phase 3 cppreflect, Phase 4 network all WISHLIST. |
...
| **Total** | | **1369** in-tree active actions across 21 in-tree namespaces (17 module namespaces + 4 framework: `monolith` meta, `level_sequence`, `bulk_fill`, `describe`). v0.17.0 adds `decision` (5 actions, Phase 1) + `risk` (5 actions, Phase 2) + 1 `source` audit action (Phase 2 cross-namespace registration `audit_module_dep_reality`) from the new `MonolithReflectionIntel` module — namespace count 19 → 21, in-tree total 1358 → 1369. Reconciled 2026-05-28: the per-namespace rows above are authoritative; the prior deferred-batch list (+1/+3/+4/+1/+17/+4/+9/+1 = +40 over the stale 1318 baseline) is now folded in. 45 town-gen experimental actions disabled by default → 1414 with `bEnableProceduralTownGen=true`. Without `WITH_COMMONUI`: 1314 (1359 w/ town gen — strips the 51 CommonUI actions then adds back the unconditional 11); without `WITH_GBA`: 1234 (1279 w/ town gen — strips the 135 GAS actions); without both: 1179 (1224 w/ town gen). Without `WITH_METASOUND`: 1357 (the 12 audio MetaSound introspection actions become inert; the `audio` bulk_fill adapter's MetaSound paths gate the same). The `ui` namespace double-counts 4 aliased GAS actions; **distinct** action handlers = **1365** in default-active configuration, **1410** with town gen registered. **Sibling plugins are NOT counted in the totals above** (per `monolith-release.md` Action Count Discipline) — the public Monolith release zip does not include them. For transparency, live `monolith_discover()` (which includes siblings if installed) returns **1592** = 1369 in-tree + 11 `claudedesign` (`MonolithClaudeDesignBridge`) + 158 `inventory` (`MonolithISX`) + 28 `steam` (`MonolithSteamBridge`) + 26 `substance` (`MonolithSubstance`); the 223 sibling actions are advisory only and never affect the public-release headline. | Namespace count rose 18 → 19 with `level_sequence` (v0.14.8); v0.14.11 [Unreleased] lit up the existing `bulk_fill` + `describe` dispatchers' adapters without adding new namespaces; v0.17.0 brings two new namespaces from MonolithReflectionIntel — `decision` (Phase 1) and `risk` (Phase 2) — for 19 → 21, plus a +1 audit on the existing `source` namespace. The 2026-05-22 `monolith_guide` addition stayed inside the existing `monolith` meta-namespace. Sibling-plugin namespaces are documented in their own repos and are NOT in the public Monolith release zip — sibling totals are advisory only |
