...
| MonolithCore | bulk_fill | 2 (Phase 0 dispatchers) **(WISHLIST — adapters Phases 1-5)** | `apply` + `list_namespaces`. Central JSON-tree write dispatcher routing on `target_namespace` to per-namespace adapters registered via `FMonolithBulkFillRegistry`. Phase 0 ships the dispatchers + framework primitives (`FMonolithReflectionWalker`, `FBulkFillSpec`, `FDryRunReport`, `FSchemaDescriptor`, `FMonolithDryRunGuard`). Per-namespace adapters (blueprint/gas/inventory/ui/ai/niagara/material/audio/mesh/animation/logicdriver/combograph) land in Phases 1-5 per plan `Docs/plans/2026-05-11-monolith-mcp-ergonomics.md`. |
| MonolithCore | describe | 2 (Phase 0 dispatchers) **(WISHLIST — adapters Phases 1-5)** | `schema` + `list_targets`. Returns rich `FSchemaDescriptor` trees (type names, ImportText forms, enum-value lists, clamp ranges, nested children). Same Phase 0 / Phase 1-5 split as `bulk_fill`. |
| **Total** | | **1303** registrations across 18 namespaces + 4 `monolith_*` meta = **1299 in-tree namespace actions** active by default (45 town-gen experimental disabled, 4 bulk_fill/describe Phase 0 dispatchers without adapters). With town gen registered: 1348. Without `WITH_COMMONUI`: 1248 (and 1293 with town gen); without `WITH_GBA`: 1168 (1213 w/ town gen); without both: 1117 (1162 w/ town gen). Without `WITH_METASOUND`: 1287 (the v0.14.10 12 introspection actions become inert) | Namespace count rose 16 → 18 with the Phase 0 framework dispatchers (`bulk_fill`, `describe`). The `ui` namespace double-counts 4 aliased GAS actions; **distinct** action handlers = **1295** in default-active configuration, **1344** with town gen registered |
