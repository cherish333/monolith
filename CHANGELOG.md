...
- **Action counts (live):** the running in-tree total lands at **1386 actions across 25 in-tree namespaces** — v0.17.0 RI brought 1358 → 1384 (Phases 1+2+3a = +16 actions / +3 namespaces; Phase 4a = +10 actions / +2 namespaces over Phase 3a's 1374 / 22), then the [Unreleased] `cppreflect_query("list_class_specifiers")` follow-up adds +1 (1384 → 1385, no new namespace), then the [Unreleased] network-completeness `reflect_query("rebuild_reflection_index")` adds +1 plus a brand-new `reflect` namespace (1385 → 1386, 24 → 25 in-tree namespaces). Live `monolith_discover()` returns **1609 total across 29 namespaces** with all four host-project sibling plugins loaded (1386 in-tree + 11 `claudedesign` + 158 `inventory` + 28 `steam` + 26 `substance` = 1609). Sibling action counts remain advisory and never affect the public Monolith release headline.
