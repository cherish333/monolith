...
- **Action counts (live):** the running in-tree total lands at **1385 actions across 24 in-tree namespaces** — v0.17.0 RI brought 1358 → 1384 (Phases 1+2+3a = +16 actions / +3 namespaces; Phase 4a = +10 actions / +2 namespaces over Phase 3a's 1374 / 22), then the [Unreleased] `cppreflect_query("list_class_specifiers")` follow-up adds +1 (1384 → 1385, no new namespace). Live `monolith_discover()` returns **1608 total across 28 namespaces** with all four host-project sibling plugins loaded (1385 in-tree + 11 `claudedesign` + 158 `inventory` + 28 `steam` + 26 `substance` = 1608). Sibling action counts remain advisory and never affect the public Monolith release headline.

- **Phase 4b (tag-graph queries + animation thread-safety audit + bare-Replicated detection) deferred alongside Phase 3b.** Three follow-on audit families are blocked on Phase 3b's tree-sitter substrate landing: `gas_query("find_tag_consumers"/"find_grant_paths"/"find_revoke_paths")` needs native-tag declaration tracking (`reflect_native_tag_decls`) to resolve native-side tag references; `animation_query("audit_thread_safety")` needs `reflect_uproperties.specifiers` populated (Phase 3a leaves these as empty strings); bare `UPROPERTY(Replicated)` detection needs either a third UHT sweep over `PropPointers[]` or a full tree-sitter substrate. All three deferred together — combined value is real but not blocking the Steam-build readiness work Phase 4a directly addresses. See `Docs/specs/SPEC_MonolithReflectionIntel.md` §9.
